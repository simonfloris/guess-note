    ...
    render() {
        ...
        return (
            <Motion
				...
				style={{x:spring(x,{...SPRING_CONFIG}),y:spring(y,{...SPRING_CONFIG}),rx:spring(ry,{...SPRING_CONFIG}),ry:spring(ry,{...SPRING_CONFIG})}}>
				{interpolatedStyle =>
            <g>
                <ellipse rx={interpolatedStyle.rx} ry={interpolatedStyle.ry} cx={interpolatedStyle.x} cy={interpolatedStyle.y} {...style}/>
                {count > 0 && [...Array(count)].map((val, index) => <line {...style} key={index} x1={interpolatedStyle.x - 20} x2={interpolatedStyle.x + 20}
